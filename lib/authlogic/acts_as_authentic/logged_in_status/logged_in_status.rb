      ...
      module Methods
        def self.included(klass)
          ...
          klass.class_eval do
           ...
           # scope :logged_out, where({"$or"=>[{:last_request => {"$lte" => logged_in_timeout.seconds.ago}}, {:last_request => nil} ]})
             scope :logged_out, where({:last_request => {"$lte" => logged_in_timeout.seconds.ago}})
          end
        end
